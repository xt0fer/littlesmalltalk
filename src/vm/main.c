...
# endif
    ...
    printf("\nGC statistics:\n");
    printf("  %lld garbage collections\n", gc_count);
    if(gc_count > 0) {
        printf("  %" PRId64 " total microseconds in GC for %lld microseconds per GC pass.\n", gc_total_time, gc_total_time/gc_count);
        printf("  %" PRId64 " microseconds for longest GC pause.\n", gc_max_time);
        printf("  %" PRId64 " total bytes copied for %lld bytes per GC on average.\n", gc_total_mem_copied, gc_total_mem_copied/gc_count);
